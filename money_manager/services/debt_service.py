...
def pay_rule_now_from_form(form) -> None:
    ...
    update_debt_rule(rule_id, {"last_generated": date.today().isoformat()})
    ...
    for row in rows:
        ...
        for due_date in _iter_due_dates_to_generate(row, today):
            ...
            append_pending({
                ...
                "category": DEBT_PAYMENT_CATEGORY,
                "account": "auto",
                "description": f"Debt instalment: {row.get('name', '')}",
                "source": "debt",
...
def page_context() -> dict:
    ...
    for rule in rules:
        ...
        rule["next_payment"] = next_due_date_for_rule(rule).isoformat()
